...
function mockHomeImage(string $seed, int $width = 800, int $height = 400): string
{
    return "https://picsum.photos/seed/{$seed}/{$width}/{$height}";
}

function mockHomeProducts(int $count, string $prefix = 'mock'): array
{
    $names = [
    ...
    for ($i = 1; $i <= $count; $i++) {
        $sku = strtoupper($prefix) . '-' . str_pad((string) $i, 3, '0', STR_PAD_LEFT);
        $price = number_format(49 + ($i * 37.5), 2, '.', '');
        $products[] = [
            'id' => 9000 + $i,
            'product_id' => 5000 + $i,
            'parent_id' => null,
            ...
            'url_key' => strtolower($prefix) . '-product-' . $i,
            'saleable' => 1,
            'new' => $i === 1 ? 1 : null,
            'html_price' => "<span class='price'>{$price} ر.س.</span>",
            'product' => [
                'base_image_url' => mockHomeImage("{$prefix}-prod-{$i}", 400, 400),
            ],
        ];
    }
...
function mockHomeReviews(): array
{
    return [
        [
            'id' => 1,
            'name' => 'سارة العتيبي',
            'rating' => 5,
            'comment' => 'تجربة شراء ممتازة، التوصيل سريع والمنتجات أصلية كما في الوصف.',
        ...
        [
            ...
            'name' => 'محمد الشهري',
            'rating' => 4,
            'comment' => 'خدمة عملاء متجاوبة وأسعار منافسة. أنصح بالتسوق من المتجر.',
        ...
        [
            ...
            'name' => 'نورة القحطاني',
            'rating' => 5,
            'comment' => 'تغليف احترافي ومنتجات عالية الجودة. سأكرر الطلب بالتأكيد.',
            'customer' => ['gender' => 'Female'],
        ],
        [
            'id' => 4,
            'name' => 'عبدالله الحربي',
            'rating' => 5,
            'comment' => 'من أفضل المتاجر الإلكترونية التي تعاملت معها. شكراً للفريق.',
            'customer' => ['gender' => 'Male'],
        ],
    ];
}

function testimonialsAll(): array
{
    return [
        'authUser' => auth(),
        'menu' => menu(),
        'reviews' => mockHomeReviews(),
        'title' => 'آراء العملاء',
        'subtitle' => 'آراء عملائنا',
    ];
}

function mockHomeBrands(): array
{
    $brands = [
        ['id' => 1, 'name' => 'رايق', 'seed' => 'brand-raq'],
        ['id' => 2, 'name' => 'نور', 'seed' => 'brand-noor'],
        ['id' => 3, 'name' => 'أصيل', 'seed' => 'brand-aseel'],
        ['id' => 4, 'name' => 'فخامة', 'seed' => 'brand-fakhamah'],
        ['id' => 5, 'name' => 'لمسة', 'seed' => 'brand-lamsa'],
        ['id' => 6, 'name' => 'سكاي', 'seed' => 'brand-sky'],
    ];
    $result = [];
    ...
    foreach ($brands as $brand) {
        $result[] = [
            'id' => $brand['id'],
            'name' => $brand['name'],
            'description' => 'ماركة تجريبية للاختبار المحلي',
            'logo' => mockHomeImage($brand['seed'], 120, 120),
            'meta_title' => $brand['name'],
            'meta_url' => strtolower($brand['name']),
            'is_active' => 1,
        ];
    }
...
function mockHomeFeatures(): array
{
    return [
        [
            'icon' => 'icon-truck',
            'main_title' => 'شحن سريع',
            'sub_title' => 'توصيل خلال ٢–٤ أيام عمل لجميع مناطق المملكة',
        ],
        [
            'icon' => 'icon-shield',
            'main_title' => 'دفع آمن',
            'sub_title' => 'بوابات دفع مشفرة ومعاملات محمية بالكامل',
        ],
        [
            'icon' => 'icon-refresh',
            'main_title' => 'استرجاع سهل',
            'sub_title' => 'سياسة استرجاع مرنة خلال ١٤ يوماً من الاستلام',
        ],
        [
            'icon' => 'icon-headphones',
            'main_title' => 'دعم ٢٤/٧',
            'sub_title' => 'فريق خدمة عملاء جاهز للمساعدة على مدار الساعة',
        ],
    ];
}

function mockHomeListItems(int $count, bool $withNames = true): array
{
    $labels = ['إلكترونيات', 'أزياء', 'منزل', 'رياضة', 'جمال', 'هدايا'];
    $items = [];

    for ($i = 1; $i <= $count; $i++) {
        $items[] = [
            'image' => mockHomeImage("list-item-{$i}", 600, 400),
            'name' => $withNames ? $labels[($i - 1) % count($labels)] : '',
            'link_type' => 'category',
...
function mockFixedProductsBlock(
    int $blockId,
    string $listType,
    string $displayType,
    string $title,
    ...
    $products = mockHomeProducts($productCount, "block{$blockId}");
    $settings = array_merge([
        'list_type' => $listType,
        'category_id' => '100',
        'sku_text' => 'MOCK-001,MOCK-002,MOCK-003,',
        'limit' => (string) $productCount,
        'type' => $displayType,
...
function homePage() {
    ...
    // بيانات تجريبية للصفحة الرئيسية — للاختبار المحلي فقط، بدون الاعتماد على منصة توسع
    $context['homeContent'] = [
        [
            'slug' => 'fixed_banner',
            'sort' => 1,
            ...
            'setting' => [],
            'settings' => [
                'image' => mockHomeImage('hero-banner', 1400, 500),
                'link' => '/offers',
            ],
        ],
        [
            ...
            'setting' => [],
            'settings' => [
                'image' => mockHomeImage('wide-promo', 1400, 350),
                'link' => '/new-arrivals',
            ],
        ],
        [
        ...
        [
            ...
            'settings' => [
                ...
                'items' => mockHomeListItems(4, true),
            ],
        ],
        [
            'slug' => 'list_of_items',
            ...
            'settings' => [
                'type' => 'slider',
                'title' => 'عروض موسمية',
                'items' => mockHomeListItems(5, true),
            ],
        ],
        mockFixedProductsBlock(100006, 'chosen_products', 'slider', 'منتجات مختارة', 5),
        mockFixedProductsBlock(100007, 'category', 'list', 'أفضل المكياج', 4, [
            'category_id' => '2122',
        ]),
        mockFixedProductsBlock(100008, 'featured_products', 'list', 'منتجات مميزة', 4, [
            'sku_text' => '',
        ]),
        mockFixedProductsBlock(100009, 'most_sales', 'slider', 'الأكثر مبيعاً', 6, [
            'sku_text' => '',
        ]),
        mockFixedProductsBlock(100010, 'latest_products', 'slider', 'أحدث المنتجات', 5, [
            'sku_text' => '',
        ]),
        [
            'slug' => 'store_features',
        ...
        [
            ...
            'settings' => [
                'brands_title' => 'الماركات',
                'link' => '/brands',
                'brand_ids' => ['1', '2', '3', '4', '5', '6'],
                'brands' => mockHomeBrands(),
            ],
        ],
        [
